...
function App() {
  ...
  const buscar = () => {
    ...
    fetch('http://tcc-guilherme.azurewebsites.net/api/monitor')
  ...
  const handleExcluir = (record: Number) => {
    ...
    fetch(`http://tcc-guilherme.azurewebsites.net/api/monitor/${idMonitor}`, {
      method: 'DELETE',
    })
      .then((response) => {
  ...
  const buscarPorNome = () => {
    ...
    setErroMessage(null);
    fetch(`http://tcc-guilherme.azurewebsites.net/api/monitornome?nome=${searchValue}`)
      .then((response) => response.json())
      .then((data) => {
        setMonitores(data);
      })
